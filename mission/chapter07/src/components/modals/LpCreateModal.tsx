...
function LpCreateModal({ onClose }: LpCreateModalProps) {
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : 'LP 생성에 실패했습니다.');
    },
  });
    ...
    >
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-[#1e1e1e] border border-white/10 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-6">
          ...
          {/* 제목 */}
          <div>
            <label htmlFor="lp-title" className="mb-1.5 block text-sm font-medium text-slate-300">
          ...
          {/* 내용 */}
          <div>
            <label htmlFor="lp-content" className="mb-1.5 block text-sm font-medium text-slate-300">
