...
export default function LpDetailPage() {
  ...
  const { mutate: toggleLike, isPending: isLiking } = useMutation({
    // 클릭 시점의 isLiked 값을 변수로 받아서 onMutate 이후에도 올바른 방향으로 API 호출
    mutationFn: (wasLiked: boolean) =>
      wasLiked ? unlikeLp(Number(lpId)) : likeLp(Number(lpId)),
    ...
    onMutate: async (wasLiked: boolean) => {
      ...
      queryClient.setQueryData<Lp>(['lp', lpId], (old) => {
        ...
        const newLikes = wasLiked
          ...
          : [...old.likes, { id: Date.now(), userId, lpId: Number(lpId) }];
