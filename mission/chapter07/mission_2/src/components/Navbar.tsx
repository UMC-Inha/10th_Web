...
const Navbar = ({ onMenuClick }: NavbarProps) => {
  ...
  return (
    <nav className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
      ...
      {/* 로고 */}
      <div
        className="font-bold text-lg cursor-pointer text-pink-400"
        onClick={() => navigate('/')}
      >
        🎵 LP판
      </div>
      ...
      <div className="flex gap-2">
        {user ? (
          <div className="flex items-center gap-3">
            <span
              className="text-gray-400 cursor-pointer hover:text-pink-400 transition-colors text-sm"
            ...
            <button
              ...
              className="px-4 py-2 text-sm border border-gray-600 text-gray-300 rounded-md hover:bg-gray-800 transition-colors"
            >
              로그아웃
            </button>
          </div>
