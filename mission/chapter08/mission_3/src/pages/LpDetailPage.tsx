...
const LpDetailPage = () => {
  ...
  const { mutate: likeMutate } = useMutation({
    mutationFn: () => {
      const alreadyLiked = lp?.likes?.some(
        (like: any) => Number(like.userId) === Number(user?.id),
      );

      return alreadyLiked ? unlikeLp(Number(lpId)) : likeLp(Number(lpId));
    },

    onMutate: async () => {
      await queryClient.cancelQueries({
        queryKey: ['lp', Number(lpId)],
      });

      const previousData = queryClient.getQueryData(['lp', Number(lpId)]);

      queryClient.setQueryData(['lp', Number(lpId)], (old: any) => {
        if (!old || !user) return old;

        const currentLp = old.data;

        const alreadyLiked = currentLp.likes?.some(
          (like: any) => Number(like.userId) === Number(user.id),
        );
    ...
    onError: (_error, _variables, context) => {
      if (context?.previousData) {
        queryClient.setQueryData(['lp', Number(lpId)], context.previousData);
      }
  ...
  const isLpOwner =
    user && lpAuthorId && Number(user.id) === Number(lpAuthorId);

  const isCommentOwner = (comment: any) => {
    const commentAuthorId = comment.authorId ?? comment.author?.id;
    return (
      user && commentAuthorId && Number(user.id) === Number(commentAuthorId)
    );
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <button
        onClick={() => navigate('/')}
        className="mb-6 text-gray-500 hover:text-gray-300 transition-colors"
      >
        ← 목록으로
      </button>
      ...
      <div className="flex gap-3 mb-10">
        <button
          ...
          className={`px-6 py-2 rounded-md transition-colors border ${
            isLiked
              ? 'bg-red-500 text-white border-red-500 hover:bg-red-600'
              : 'text-white border-gray-600 hover:bg-red-900/30'
          }`}
        >
          ❤️ {lp.likes?.length ?? 0}
        </button>
        ...
        {!isCommentsLoading && (
          <div className="flex flex-col gap-4">
            {comments.map((comment: any) => (
              <div
                key={comment.id}
                className="flex flex-col gap-1 border-b border-gray-800 pb-4"
              >
