...
export async function searchLps(search: string, order: SortOrder = 'desc', cursor: number = 0, limit: number = 20): Promise<LpListResponse> {
  const { data } = await axiosInstance.get('/v1/lps', { params: { search, order, cursor, limit } });
  return data.data;
}
