  ...
  return (
    <nav className="fixed top-0 left-0 right-0 h-14 bg-[#111] flex items-center justify-between px-6 z-100 border-b border-[#222]">
      ...
      <div className="flex gap-2 items-center">
        {isLoggedIn ? (
          <button
            className="px-[14px] py-[6px] rounded-md text-sm font-semibold cursor-pointer transition-opacity hover:opacity-80 bg-transparent border border-[#555] text-white disabled:opacity-50"
            onClick={() => doLogout()}
            disabled={isPending}
          >
